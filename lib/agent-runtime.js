...
async function buildToolCatalog() {
  ...
  if (fs.existsSync(manifestDir)) {
    ...
    for (const tool of compiled) {
      ...
      const cred = await secrets.resolve(tool.provider);
      const authed = !!cred;
      tools.push({ ...tool, authed, args: tool.input_schema?.properties || {} });
    }
  }
  ...
  if (apiTools.length) {
    ...
    // Use compiler's catalog formatter for rich param info
    catalog += toCatalogString(apiTools, { showParams: true });
  } else {
...
class AgentRuntime {
  ...
  async run(goal) {
    ...
    // Planner passes messages array: [{role, content}, ...]
    ...
    const planner = new Planner({
      ...
      maxReplanCycles: 3,
      dryRun:      this.dryRun,
      semanticVerify: !this.dryRun, // skip expensive LLM validation in dry mode
    });
    ...
    try {
      result = await planner.run(goal, {
        ...
        catalog,   // inject tool catalog so planner injects it into LLM prompts
      });
    } finally {
