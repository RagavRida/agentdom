...
function _writeFile(map) {
  _ensureDir();
  atomicWrite(WALLET_FILE, JSON.parse(_encrypt(map)), { mode: 0o600 });
  // Actually write the raw encrypted string, not re-serialized object
  const encrypted = _encrypt(map);
  fs.writeFileSync(WALLET_FILE + '.tmp', encrypted, { mode: 0o600 });
  fs.renameSync(WALLET_FILE + '.tmp', WALLET_FILE);
}
