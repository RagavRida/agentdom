...
class ReactiveAgent {
  ...
  async run(goal) {
    ...
    for (let step = 1; step <= this.maxSteps; step++) {
      ...
      // ── 2. Build LLM context ─────────────────────────────────────────────────
      const systemPrompt = this._buildSystemPrompt(catalog, goal);
  ...
  _buildSystemPrompt(catalog, goal) {
    // Filter catalog to goal-relevant tools only (keeps prompt under 6000 chars)
    const filteredCatalog = this._filterCatalog(catalog, goal);
...
- Use only tools listed above (copy the intent name exactly).
- Args must match the tool's parameter types exactly.
- If a previous result contains an ID or number, reference it directly in args.
- "done": true only when the goal is verifiably complete.`;
  }

  /**
   * Filter the full catalog down to the ~15 most relevant tools for the goal.
   * Uses keyword matching on the goal string against intent names and descriptions.
   * Keeps catalog under ~6000 chars for reliable LLM processing.
   */
  _filterCatalog(catalog, goal) {
    if (!goal || !catalog) return catalog?.slice(0, 6000) || '';

    // Extract keywords from goal (nouns + verbs)
    const stopWords = new Set(['a','an','the','in','on','at','to','for','of','and','or','with','that','this','it','its']);
    ...
    const score = (line) => {
      ...
      return keywords.reduce((s, kw) => s + (l.includes(kw) ? 2 : 0), 0)
        + (l.includes('builtin') || l.includes('browser') ? 1 : 0);
    };

    // Always include headers and builtins
    const result = [];
    let inBuiltin = false;
    for (const line of lines) {
      if (line.startsWith('###') || line.startsWith('##')) {
        inBuiltin = line.toLowerCase().includes('built-in') || line.toLowerCase().includes('browser');
        result.push(line);
        continue;
      }
      if (inBuiltin) { result.push(line); continue; }
      if (score(line) > 0) result.push(line);
    }

    const filtered = result.join('\n');
    // If filtering left too little, fall back to raw slice
    if (filtered.length < 200) return catalog.slice(0, 6000);
    return filtered.slice(0, 6000);
  }
  ...
  _parseDecision(raw) {
    ...
    try {
      ...
      // Handle LLM wrapping call in dispatch_intent({ intent, params })
      if (!d.tool && d.intent) {
        return {
          reasoning:      d.reasoning      || '',
          tool:           d.intent,
          args:           d.params || d.args || {},
          done:           d.done === true,
          result_summary: d.result_summary || '',
        };
      }
      // Handle nested dispatch_intent object: { "tool": "dispatch_intent", "args": { "intent": ..., "params": ... } }
      if (d.tool === 'dispatch_intent' && d.args?.intent) {
        return {
          reasoning:      d.reasoning      || '',
          tool:           d.args.intent,
          args:           d.args.params || d.args.args || {},
          done:           d.done === true,
          result_summary: d.result_summary || '',
        };
      }
      ...
      // Fallback: extract from malformed response
      const toolMatch   = raw.match(/"tool"\s*:\s*"([^"]+)"/);
      const intentMatch = raw.match(/"intent"\s*:\s*"([^"]+)"/);
      const argsMatch   = raw.match(/"(?:args|params)"\s*:\s*(\{[^}]*\})/);
      return {
        reasoning: 'parsed from malformed response',
        tool:  toolMatch?.[1] || intentMatch?.[1] || null,
        args:  argsMatch ? (() => { try { return JSON.parse(argsMatch[1]); } catch { return {}; } })() : {},
 ...
 * Accepts a pre-built catalog to avoid rebuilding on every scan step.
 *
 * @param {function} llm
 * @param {function} buildCatalog
 * @param {function} dispatch
 * @param {object}   [prebuiltCatalog]  — pass { catalog, authedTools, allTools } to skip rebuild
 * @param {object}  [opts]
 * @returns {ReactiveAgent}
 */
function createAgent({ llm, buildCatalog, dispatch, prebuiltCatalog, ...opts }) {
  // Snapshot the catalog once — returned instantly on every scan() call
  let catalogSnapshot = prebuiltCatalog || null;
  ...
  return new ReactiveAgent({
    ...
    scan: async () => {
      // Only build once; subsequent steps reuse the snapshot (catalog is cached anyway)
      if (!catalogSnapshot) {
        catalogSnapshot = await buildCatalog();
      }
      return {
        catalog: catalogSnapshot.catalog,
        tools:   catalogSnapshot.authedTools,
      };
    },

    execute: async (toolName, args) => {
      // Resolve tool → intent + provider from the snapshot
      const allTools = catalogSnapshot?.allTools || [];
