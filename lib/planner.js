...
- Args can reference previous step results with $s1.data.id syntax.`;
   ...
   */
  _verify(step) {
    // First: if result explicitly says ok:false with no verify expression, treat as failed
    if (!step.verify && step.result?.ok === false) {
      return { passed: false, reason: step.result.error || 'Step returned ok:false' };
    }
