    ...
    ws.on('open', async () => {
      try {
        ...
        if (intent === 'browser.navigate' || intent === 'navigate') {
          ...
          // Wait for React/SPA hydration (GitHub needs ~3s after navigate)
          ...
          const r = await send('Runtime.evaluate', {
            expression: `(function(){
              ...
              // Dedicated inputs array — the LLM needs exact selectors
              const inputs = [];
              ...
              return { title: document.title, url: location.href, text: document.body.innerText.slice(0,1000), elements: els.slice(0,30), inputs };
            })()`,
            returnByValue: true,
          });
          data = r.result?.value || {};
          ...
          // Try multiple selector strategies — LLM often guesses wrong selector format
          const hint   = args.label || args.name || '';
          ...
          const r = await send('Runtime.evaluate', {
            expression: `(function(){
              ...
              const strategies = [
                sel,
                '[placeholder="Title"]',                          // GitHub new issue (stable)
                '#issue_title', '#issue_body',                         // legacy GitHub IDs
                '[placeholder*="description"]', '[placeholder*="Type your"]', // body
                sel ? sel.replace(/^input\\[name=/, 'textarea[name=') : null,
                hint ? '[placeholder*="' + hint + '"]' : null,
                hint ? '[aria-label*="' + hint + '"]' : null,
                hint ? '[name="' + hint + '"]' : null,
              ...
              for (const s of strategies) {
                try { const found = document.querySelector(s); if (found) { el = found; break; } } catch {}
              }
              if (!el && hint) {
                const labels = Array.from(document.querySelectorAll('label'));
                const lbl = labels.find(l => l.textContent.trim().toLowerCase().includes(hint.toLowerCase()));
                if (lbl && lbl.htmlFor) el = document.getElementById(lbl.htmlFor);
              }
              // Last resort: first editable input
              if (!el) el = document.querySelector('input:not([type=hidden]):not([type=submit]):not([type=button]), textarea');
              if (!el) return { error: 'Input not found', tried: strategies.slice(0,5) };
              el.focus();
              el.value = '';
              el.value = ${JSON.stringify(text)};
              el.dispatchEvent(new InputEvent('input',  { bubbles: true }));
              el.dispatchEvent(new Event('change', { bubbles: true }));
              return { typed: true, into: el.id || el.name || el.placeholder || el.tagName };
            })()`,
            returnByValue: true,
          });
          data = r.result?.value || {};
          if (data.error) process.stderr.write(`[CDP] fill: ${data.error}\n`);
        }
