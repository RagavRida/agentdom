...
function fromWallet(host) {
  try {
    ...
    // Check OAuth token expiry and use refresh token if needed
    if (entry.method === 'oauth2' && entry.expires_at) {
      const expiresAt = new Date(entry.expires_at);
      if (expiresAt < new Date() && !entry.refresh_token) return null; // expired, no refresh
    }
    return { ...entry, source: 'wallet', host };
  } catch { return null; }
}
