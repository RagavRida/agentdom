 ...
 * 
 * Resolution order:
 *  0. Agent Token Protocol (publisher-issued scoped tokens — best option)
 *  1. Env vars
 *  2. Wallet file (AGENTDOM_WALLET_B64 > AGENTDOM_WALLET_PATH > ~/.agentdom/wallet.json)
 *  3. OS Keychain
 *  4-6. External vaults (AWS SSM, HashiCorp Vault, 1Password)
 *
...
async function resolve(host) {
  ...
  // 0. Agent Token Protocol — publisher issues scoped tokens directly to the agent
  //    Only if: publisher supports it AND we have a master credential
  try {
    const { ensureAgentToken, getAgentTokenSpec } = require('./agent-tokens');
    const spec = await getAgentTokenSpec(host);
    if (spec) {
      const walletMaster = fromWallet(host);
      const envMaster = fromEnv(host);
      if (walletMaster || envMaster) {
        // We have a master token AND publisher supports agent_tokens — use protocol
        const agentTok = await ensureAgentToken(host, { agentId: 'agentdom-runtime' });
        if (agentTok?.token) {
          return { method: 'api_key', token: agentTok.token, source: 'agent_tokens_protocol', host };
        }
      }
    }
  } catch (_) { /* protocol not supported or publisher unreachable — fall through */ }
